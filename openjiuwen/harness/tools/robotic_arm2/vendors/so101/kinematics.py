...
"""Shared geometry/IK helpers for the SO-101 ``ArmBackend``.

Ported unchanged (math-for-math) from ``robotic_arm.vendors.so101._kinematics``
-- this is the same pixel<->world math, ``IKSolver``, and reachability checks,
just re-homed under ``robotic_arm2`` so the tool-based rewrite has no runtime
dependency on the ``robotic_arm`` package. See ``backend.py`` for how the
individual functions here back each ``ArmBackend`` method / LLM tool.
...
def pixel_from_normalized(x: float, y: float, scale: int, width: int, height: int) -> tuple[int, int]:
    """Convert a normalized point in ``[0, scale]`` to a pixel on ``width``x``height``."""
    px = round(x / scale * width)
    py = round(y / scale * height)
...
def check_reachability(
    ...
    """Single-point, single-elevation IK feasibility check (used by ``check_reachability`` tool)."""
...
def check_keypoint_reachability(
    ...
    """Per-keypoint IK error at a range of approach elevations."""
    report = []
...
def reachability_report_str(report: list[dict]) -> str:
    """Format :func:`check_keypoint_reachability`'s output as text."""
    lines = []
    for r in report:
        errs = "  ".join(
...
class IKSolver:
    ...
    def __init__(
        ...
        *,
        base_elements: Sequence[str] | None = None,
