#!/usr/bin/env python
"""Tests for the pure geometry/reachability helpers in kinematics.py.

Ported unchanged from ``tests/unit_tests/harness/tools/robotic_arm/vendors/so101/test_kinematics.py``
(the underlying math didn't change, only the module's location) -- these never
need ikpy/scipy installed (they're plain numpy); only constructing
:class:`IKSolver` itself does, and that path is covered separately with a
simulated missing-dependency import failure.
"""
...
from openjiuwen.harness.tools.robotic_arm2.vendors.so101 import kinematics
